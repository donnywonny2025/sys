...
(function() {
  ...
  DASH.addChatFeedBubble = function(text, style, ts) {
    ...
    if (isOut) {
      ...
      var sender = document.createElement('div'); sender.className = 'chat-sender'; sender.textContent = 'OPENCLAW'; bubble.appendChild(sender);
  ...
  fetch('/api/history')
    ...
    .then(function(history) {
      ...
      if (consoleItems.length === 0) DASH.addConsoleLine('Console initialized. Waiting for OpenClaw activity...', 'sys');
      setTimeout(function() {
    ...
    .catch(function() { DASH.addConsoleLine('Console initialized. Waiting for OpenClaw activity...', 'sys'); });
})();
