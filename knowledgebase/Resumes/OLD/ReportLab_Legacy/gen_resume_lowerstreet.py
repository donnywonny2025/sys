...
def draw_wrapped_at(c, text, x, y_baseline, font, size, color, max_width, leading, is_bullet=False):
    ...
    If is_bullet is True, the first line is drawn at (x), and subsequent 
    lines are indented to match the first character of the text.
    Actually, for hanging indents, we draw the bullet at x and the TEXT lines at text_x.
    """
    c.setFont(font, size)
    c.setFillColor(color)
    
    if is_bullet:
        # Handle the bullet character and calculate the indent for text lines
        if text.startswith("• "):
            text = text[2:]
        bullet_char = "•  "
...
def build_resume(output_path, monogram_path):
    ...
    c.drawString(136.9, ytop_to_baseline(59.6, 10), "Senior Video Editor / Creative Producer")
    ...
    summary = (
        "Senior Video Editor and Creative Producer with extensive experience in narrative editing, YouTube growth, and end-to-end podcast post-production. I specialize in turning raw interview footage and rough cuts (including Descript workflows) into high-fidelity, polished content for enterprise brands and global agencies. I focus on elevating stories through precise color correction, motion graphics, and social-first platform versioning without missing a deadline."
    )
    ...
    skills = [
        ("Podcast/YT:", "Podcast Post-Production, YouTube Growth Strategy, Narrative Editing, Platform Versioning"),
        ("Video/Graphics:", "Video Editing Software, Motion Graphics, Color Correction/Grading, Audio Polishing"),
        ("Post-Workflow:", "Rough-Cut Handoffs (Descript), Frame.io Review Cycles, Technical QC, B-Roll Sourcing"),
        ("Writing:", "Interview Direction, Scripting, Advocacy Content, Technical Write-ups"),
        ("Tools:", "Premiere Pro, After Effects, DaVinci Resolve, Frame.io, Logic Pro, Descript (familiar), VS Code, Google, Git"),
        ("AI:", "Veo, Sora, Kling, Flux, integrating AI models into modern production pipelines"),
    ]
    ...
    for label, content in skills:
        ...
        # Use a FIXED column width for all content to prevent ragged edges
    ...
    j1_bullets = [
        "Develop and edit high-end YouTube and podcast content for enterprise brands and professional agencies",
        "Lead projects end-to-end with clear milestones, delivering high-fidelity video finishing and grading",
        "Utilize modern post-production tools (Descript, Premiere, Frame.io) to support rapid-cycle delivery",
...
OUTPUT = '/Volumes/WORK 2TB/SAVE/AGENTMONEY/KNOWLEDGE_BASE/Resumes/Jeff_Kerr_Resume_LowerStreet.pdf'
