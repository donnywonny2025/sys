"""
Ethos Life Application (Cover Letter + Resume)
Resume page uses the EXACT same coordinate system as gen_resume_general.py.
Rules: No em-dashes.
"""
...
def draw_branded_header(c, monogram_path):
    """Draw the branded header - identical across all resume variants."""
...
# PAGE 1: COVER LETTER
# ─────────────────────────────────────────────────
def draw_cover_letter(c, monogram_path):
    draw_branded_header(c, monogram_path)

    # Letter body starts below the orange line
    letter_y = ytop_to_baseline(140.0, 10)

    c.setFont('Helvetica', 10)
    c.setFillColor(BODY_CLR)

    c.drawString(LEFT, letter_y, "To the Ethos Hiring Team,")
    letter_y -= BODY_LEADING * 2

    p1 = (
        "I am writing to express my interest in the Video Editor position at Ethos Life. "
        "I spent some time researching Ethos, and your mission to protect families by democratizing "
        "access to life insurance really aligns with what I am looking for. In an increasingly "
        "fast-changing world, building an authentic, accessible experience is critical, and that "
        "requires video content that builds immediate trust."
    )
    letter_y = draw_wrapped_at(c, p1, LEFT, letter_y, 'Helvetica', 10, BODY_CLR, CONTENT_W, BODY_LEADING)
    letter_y -= BODY_LEADING

    p2 = (
        "With over 15 years of experience producing and editing high-stakes campaigns for brands "
        "like Disney, Boeing, and the Department of Defense, I have developed the technical precision "
        "and storytelling fundamentals necessary to create compelling assets. More recently, I have "
        "successfully applied these standards to the YouTube and paid social ecosystems, utilizing "
        "the fast pacing and strong hooks required for effective direct response."
    )
    letter_y = draw_wrapped_at(c, p2, LEFT, letter_y, 'Helvetica', 10, BODY_CLR, CONTENT_W, BODY_LEADING)
    letter_y -= BODY_LEADING

    p3 = (
        "My complete portfolio of work is available at jefferykerr.com. I would welcome the "
        "opportunity to discuss how my editorial workflow and dedication to quality can support "
        "the marketing and growth objectives at Ethos Life."
    )
    letter_y = draw_wrapped_at(c, p3, LEFT, letter_y, 'Helvetica', 10, BODY_CLR, CONTENT_W, BODY_LEADING)
    letter_y -= BODY_LEADING * 3

    c.drawString(LEFT, letter_y, "Best regards,")
    letter_y -= BODY_LEADING * 2

    c.setFont('Helvetica-Bold', 10)
    c.drawString(LEFT, letter_y, "Jeffery Kerr")
    letter_y -= BODY_LEADING
    c.setFont('Helvetica', 10)
    c.drawString(LEFT, letter_y, "colour8k@mac.com")
    letter_y -= BODY_LEADING
    c.drawString(LEFT, letter_y, "407-620-3618")


# ─────────────────────────────────────────────────
# PAGE 2: RESUME (identical layout to gen_resume_general.py)
# ─────────────────────────────────────────────────
def draw_resume(c, monogram_path):
    draw_branded_header(c, monogram_path)
    ...
    summary = (
        "I lead video campaigns across brands, corporate, and the public sector: launch pieces, explainers, interview-driven stories, and big event packages. I turn fuzzy ideas into clear, watchable work and keep delivery predictable. Trusted by Disney, Boeing, Century 21, Raytheon, DoD, FTC, and New Balance. I assemble and lead the right crew, and I still cut when it helps. I work with modern tooling, including AI workflows, custom API integrations, and web application development."
    )
    ...
    skills = [
        ("Production & Direction:", "Briefs, Treatments, Budgets, Crew Management, Stakeholder Reviews"),
        ("Editorial:", "Narrative Editing, Platform Versioning, QC/Captions"),
        ("Writing:", "Interview/VO Direction, Scripting, Compliance"),
        ("Creative Dev:", "React, NextJS, Three.js, Puppeteer, Browser Automation, API Integration, Git"),
        ("Tools:", "Premiere Pro, After Effects, Cinema 4D, DaVinci Resolve, Frame.io, Logic Pro, VS Code, Google, Git"),
        ("AI:", "Video Generation (Google Veo3, Sora, Kling), Image Generation (Flux), Integrating AI Models into Production Workflows"),
    ]
    ...
    j1_bullets = [
        "Produce and direct video campaigns: launch pieces, explainers, interviews, and large event packages",
        "Lead projects end-to-end with clear milestones, delivering broadcast-quality edits for Fortune 500 brands",
        "Build internal workflow tools and API pipelines to automate processes and accelerate video delivery times",
        "Design and deploy custom web applications utilizing modern scripting and browser automation",
    ]
...
def build_application(output_path, monogram_path):
    c = canvas.Canvas(output_path, pagesize=letter)
    draw_cover_letter(c, monogram_path)
    c.showPage()
    draw_resume(c, monogram_path)
    c.save()
    print(f"PDF saved: {output_path}")


MONOGRAM = '/Volumes/WORK 2TB/SAVE/AGENTMONEY/_archive/monogram_iterations/JKMONO.png'
OUTPUT = '/Volumes/WORK 2TB/SAVE/AGENTMONEY/KNOWLEDGE_BASE/Resumes/Jeff_Kerr_Application_Ethos.pdf'

os.makedirs(os.path.dirname(OUTPUT), exist_ok=True)
build_application(OUTPUT, MONOGRAM)
